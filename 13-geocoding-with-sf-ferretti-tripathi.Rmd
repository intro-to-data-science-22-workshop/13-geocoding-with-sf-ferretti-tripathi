...
```{r, results='hide',  message=FALSE, warning=FALSE}
#Get the addresses of the German ministries located in Berlin: 

berlin_ministries <- c("Willy-Brandt-Strasse 1, 10557 Berlin",
...
```
---
[This website](https://www.diva-gis.org/gdata) is a very good resource for free geographic (GIS) data for any country in the world
...
#Find the shapefile and then save it as a dataframe
germany_shp <- sf::st_read("DEU_adm1.shp")
view(germany_shp)
```

Note that in the "geometry" column of the dataset, you have the geometry type (multipolygon) and the coordinates (latitude and longitude)
...
coords <- coordinates_br %>%
  sf::st_as_sf(coords = c("lng", "lat"), crs = 84)
coord_geo %>%
